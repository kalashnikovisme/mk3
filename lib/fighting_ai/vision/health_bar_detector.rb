  ...
  module Vision
    ...
    class HealthBarDetector
      ...
      def detect(frame_observation)
        pixels     = frame_observation.pixels
        img_width  = frame_observation.width
        {
          1 => measure(@bar1, pixels, img_width),
          2 => measure(@bar2, pixels, img_width)
        }
      end

      private

      def measure(bar, pixels, img_width)
